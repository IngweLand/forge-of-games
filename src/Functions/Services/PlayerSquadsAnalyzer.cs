...
public interface IPlayerSquadsAnalyzer
{
    Task Analyze();
}

public class PlayerSquadsAnalyzer(IFogDbContext context, ILogger<IPlayerSquadsAnalyzer> logger) : IPlayerSquadsAnalyzer
{
    ...
    public async Task Analyze()
    {
        ...
        await CreateEquipmentInsights(squads);
        await CreateRelicInsights(squads);
    }

    private async Task CreateRelicInsights(IReadOnlyCollection<ProfileSquadDataEntity> squads)
    {
        var relics = new List<RelicStatItem>();
        foreach (var squad in squads)
        {
            var hero = squad.Hero;
            var abilities = hero.Abilities.ToList();
            if (abilities.Count < 2)
            {
                continue;
            }

            var relic = HohStringParser.GetConcreteId(abilities[1]).Split('_')[0];
            relics.AddRange(new RelicStatItem()
            {
                UnitId = hero.UnitId,
        ...
        var groupsByUnit = relics.GroupBy(x => x.UnitId);
        var relicInsights = new List<RelicInsightsEntity>();

        foreach (var g in groupsByUnit)
        {
            logger.LogInformation("Processing insights for unit {UnitId}", g.Key);

            var byLevelRange = g.GroupBy(x => x.LevelRange);
            relicInsights.AddRange(byLevelRange.Select(lrg => new RelicInsightsEntity
            {
                UnitId = g.Key,
                FromLevel = lrg.Key.From!.Value,
                ToLevel = lrg.Key.To!.Value,
                Relics = GetTopItems(lrg, x => x.Relic),
            }));
        }

        await context.RelicInsights.ExecuteDeleteAsync();

        context.RelicInsights.AddRange(relicInsights);

        logger.LogInformation("Saving relic insights to database");
        await context.SaveChangesAsync();
        logger.LogInformation("Equipment relic saved successfully");
    }

    private async Task<IReadOnlyCollection<ProfileSquadDataEntity>> GetSquads()
    {
        var result = new List<ProfileSquadDataEntity>();
        var i = 0;
    ...
    private async Task CreateEquipmentInsights(IReadOnlyCollection<ProfileSquadDataEntity> squads)
    {
        ...
        foreach (var squad in squads)
        {
            ...
            mainAttributes.AddRange(hero.Equipment.Select(equipmentItem => new EquipmentStatItem
            {
                UnitId = hero.UnitId,
                LevelRange = GetLevelRange(hero.Level),
        ...
        var equipmentInsights = new List<EquipmentInsightsEntity>();

        foreach (var g in mainGroupsBySlot)
        {
            logger.LogInformation("Processing insights for unit {UnitId} slot {SlotType}", g.Key.UnitId,
                g.Key.SlotType);

            var byLevelRange = g.GroupBy(x => x.LevelRange);
            equipmentInsights.AddRange(byLevelRange.Select(lrg => new EquipmentInsightsEntity
            {
                UnitId = g.Key.UnitId,
        ...
        context.EquipmentInsights.AddRange(equipmentInsights);
