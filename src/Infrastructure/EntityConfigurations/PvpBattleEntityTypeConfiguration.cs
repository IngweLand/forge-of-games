...
public class PvpBattleEntityTypeConfiguration : IEntityTypeConfiguration<PvpBattle>
{
    ...
    {
        builder.ToTable("pvp_battles",
            t => { t.HasCheckConstraint("CK_pvp_battles_WinnerId_LoserId_Different", "[WinnerId] <> [LoserId]"); });
        ...
        builder.HasOne(x => x.Teams).WithOne(x => x.PvpBattle).HasForeignKey<PvpBattleTeams>(x => x.PvpBattleId)
            .IsRequired();
    }
}
